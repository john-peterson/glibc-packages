...
TERMUX_PKG_DESCRIPTION="requires chroot proot . contains unfinished  root.conf for non proot gapt command "
...
termux_step_post_make_install() {
	...
	# non proot paths incomplete 
	sed -e "s|@TERMUX_PREFIX@|${TERMUX_PREFIX}|g" \
		-e "s|@TERMUX_CACHE_DIR@|${TERMUX_CACHE_DIR}|g" \
	$TERMUX_PKG_BUILDER_DIR/root.conf.in \
	> $TERMUX_PKG_BUILDER_DIR/root.conf
	cp $TERMUX_PKG_BUILDER_DIR/root.conf $target/
