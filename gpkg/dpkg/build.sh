...
termux_step_post_massage() {
	set +e
	test -d bin && bin=bin
	test -d usr/bin && bin=usr/bin

	# restore --root functionality 
	mkdir libexec
	bash $TERMUX_PKG_BUILDER_DIR/replace $bin/dpkg $PREFIX/var/lib/dpkg /var/lib/dpkg
	mv $bin/dpkg libexec/
