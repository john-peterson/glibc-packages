...
termux_step_make_install32() {
	...
	declare -A HEADERS
	IFS=$'\n'
	for h in $(find ${TERMUX_PREFIX}/include -type f -exec md5sum "{}" \;); do
		HEADERS[$(awk '{printf $2}' <<< "$h" | sed "s|${TERMUX_PREFIX}/include/||g")]+=$(awk '{printf $1}' <<< "$h")
	done
	unset IFS
	for hpath in $(find ${TERMUX_PKG_BUILD32DIR}/${TERMUX_PREFIX}/include -type f); do
		h=$(sed "s|${TERMUX_PKG_BUILD32DIR}/${TERMUX_PREFIX}/include/||g" <<< "$hpath")
		grep -q " ${h} " <<< " ${!HEADERS[@]} " || continue
		if [ "${HEADERS[${h}]}" = "$(md5sum ${hpath} | awk '{printf $1}')" ]; then
			rm "${hpath}"
		fi
	done
